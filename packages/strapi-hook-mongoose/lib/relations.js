  ...
  update: async function (params) {
    ...
    const values = params.parseRelationships === false ? params.values : Object.keys(removeUndefinedKeys(params.values)).reduce((acc, current) => {
      ...
      switch (association.nature) {
          ...
          const updatePromise = assocModel.updateMany({
            ...
            .then(() => {
              return assocModel.updateMany({
                [assocModel.primaryKey]: {
                  $in: (Array.isArray(property) ? property : JSON.parse(property)).map(val => new mongoose.Types.ObjectId(val[assocModel.primaryKey]||val))
                }
        ...
        case 'manyToMany': {
          if (details.dominant) {
            return _.set(acc, current, (Array.isArray(property) ? property : JSON.parse(property)).map(val => val[assocModel.primaryKey] || val));
          }
